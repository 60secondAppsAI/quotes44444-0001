...
const loadWishlists = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/wishlist/wishlists?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadWishlists(currentPage.value + 1);
  }
};
...
const getWishlistIcon = (wishlist: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[wishlist.wishlistId % icons.length];
};

const viewWishlistDetails = (wishlist: any) => {
  router.push({ name: 'WishlistDetail', params: { id: wishlist.wishlistId } });
};
...
<template>
  <div class="wishlists-page">
    ...
    <header class="wishlists-header">
      <div class="header-row">
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              <h1 class="page-title">Wishlists</h1>
              <div class="wishlists-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="wishlists-content">
      ...
      <div v-if="isLoading" class="loading-state">
        ...
        <p class="loading-text">Loading wishlists...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="error-state">
        <div class="error-icon">⚠️</div>
        <h3 class="error-title">Failed to Load Wishlists</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadWishlists()" class="retry-button">
          Try Again
        </button>
      </div>

      <!-- Empty State -->
      <div v-else-if="filteredWishlists.length === 0" class="empty-state">
        <div class="empty-icon">👥</div>
        <h3 class="empty-title">No Wishlists Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No wishlists match your search criteria.' : 'No wishlists found in the system.' }}
        </p>
      </div>

      <!-- Wishlists Grid -->
      <div v-else-if="viewMode === 'cards'" class="wishlists-grid">
        <div
          v-for="wishlist in filteredWishlists"
          :key="wishlist.wishlistId"
          class="wishlist-card"
          @click="viewWishlistDetails(wishlist)"
        >
          <div class="wishlist-header">
            <div class="wishlist-icon">
              {{ getWishlistIcon(wishlist) }}
            </div>
          </div>

          <div class="wishlist-info">
            <h3 class="wishlist-name">{{ wishlist.name || 'Unknown Wishlist' }}</h3>
            <p class="wishlist-description">
              Wishlist ID: {{ wishlist.wishlistId }}
            </p>
            
            <div class="wishlist-metadata">

              <div class="metadata-item">
                <span class="metadata-label">WishlistId:</span>
                <span class="metadata-value">{{ wishlist.wishlistId || 'N/A' }}</span>
              </div>
      ...
      <!-- Wishlists Table -->
      <div v-else-if="viewMode === 'table'" class="wishlists-table-container">
        <table class="wishlists-table">
          <thead>
            <tr>

              <th @click="handleSort('wishlistId')" class="sortable-header">
                WishlistId {{ getSortIcon('wishlistId') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="wishlist in filteredWishlists" :key="wishlist.wishlistId" class="wishlist-row">
              
              
                      <td class="wishlist-id-cell" @click="viewWishlistDetails(wishlist)">{{ wishlist.wishlistId }}</td>
