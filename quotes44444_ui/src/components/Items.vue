...
const loadItems = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/item/items?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const getItemIcon = (item: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[item.itemId % icons.length];
};

const viewItemDetails = (item: any) => {
  router.push({ name: 'ItemDetail', params: { id: item.itemId } });
};
  ...
  <div class="items-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="items-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="items-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <button @click="loadItems()" class="retry-button">
          Try Again
        </button>
      </div>
      ...
      <div v-else-if="filteredItems.length === 0" class="empty-state">
        ...
        <p class="empty-message">
          {{ searchQuery ? 'No items match your search criteria.' : 'No items found in the system.' }}
        </p>
      </div>

      <!-- Items Grid -->
      <div v-else-if="viewMode === 'cards'" class="items-grid">
        <div
          ...
          <div class="item-info">
            ...
            <div class="item-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">ItemId:</span>
                <span class="metadata-value">{{ item.itemId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Name:</span>
                <span class="metadata-value">{{ item.name || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Description:</span>
                <span class="metadata-value">{{ item.description || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">InitialPrice:</span>
                <span class="metadata-value">{{ item.initialPrice || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">AuctionEndDate:</span>
                <span class="metadata-value">{{ item.auctionEndDate || 'N/A' }}</span>
              </div>
      ...
      <!-- Items Table -->
      <div v-else-if="viewMode === 'table'" class="items-table-container">
        <table class="items-table">
          <thead>
            <tr>

              <th @click="handleSort('itemId')" class="sortable-header">
                ItemId {{ getSortIcon('itemId') }}
              </th>
              ...
              <th @click="handleSort('initialPrice')" class="sortable-header">
                InitialPrice {{ getSortIcon('initialPrice') }}
              </th>
              <th @click="handleSort('auctionEndDate')" class="sortable-header">
                AuctionEndDate {{ getSortIcon('auctionEndDate') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="item in filteredItems" :key="item.itemId" class="item-row">
              
              
                      <td class="item-id-cell" @click="viewItemDetails(item)">{{ item.itemId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ item.name }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ item.description }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ item.initialPrice }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ item.auctionEndDate }}</span>
                </div>
              </td>
