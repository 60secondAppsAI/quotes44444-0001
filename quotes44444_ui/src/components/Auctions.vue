...
const loadAuctions = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/auction/auctions?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadAuctions(currentPage.value + 1);
  }
};
...
const getAuctionIcon = (auction: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[auction.auctionId % icons.length];
};

const viewAuctionDetails = (auction: any) => {
  router.push({ name: 'AuctionDetail', params: { id: auction.auctionId } });
};
...
<template>
  <div class="auctions-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="auctions-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="auctions-content">
      <!-- Loading State -->
      <div v-if="isLoading" class="loading-state">
        <div class="loading-spinner-large"></div>
        <p class="loading-text">Loading auctions...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="error-state">
        <div class="error-icon">⚠️</div>
        <h3 class="error-title">Failed to Load Auctions</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadAuctions()" class="retry-button">
          Try Again
        </button>
      </div>
      ...
      <div v-else-if="filteredAuctions.length === 0" class="empty-state">
        ...
        <h3 class="empty-title">No Auctions Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No auctions match your search criteria.' : 'No auctions found in the system.' }}
        </p>
      </div>

      <!-- Auctions Grid -->
      <div v-else-if="viewMode === 'cards'" class="auctions-grid">
        <div
        ...
        >
          <div class="auction-header">
            <div class="auction-icon">
              {{ getAuctionIcon(auction) }}
            </div>
          </div>

          <div class="auction-info">
            <h3 class="auction-name">{{ auction.name || 'Unknown Auction' }}</h3>
            ...
            <div class="auction-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">AuctionId:</span>
                <span class="metadata-value">{{ auction.auctionId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">StartDate:</span>
                <span class="metadata-value">{{ auction.startDate || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">EndDate:</span>
                <span class="metadata-value">{{ auction.endDate || 'N/A' }}</span>
              </div>
      ...
      <!-- Auctions Table -->
      <div v-else-if="viewMode === 'table'" class="auctions-table-container">
        <table class="auctions-table">
          <thead>
            <tr>

              <th @click="handleSort('auctionId')" class="sortable-header">
                AuctionId {{ getSortIcon('auctionId') }}
              </th>
          ...
          <tbody>
            <tr v-for="auction in filteredAuctions" :key="auction.auctionId" class="auction-row">
              
              
                      <td class="auction-id-cell" @click="viewAuctionDetails(auction)">{{ auction.auctionId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ auction.startDate }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ auction.endDate }}</span>
                </div>
              </td>
...
.auctions-table th:first-child,
.auctions-table td:first-child {
  width: 140px;
  min-width: 140px;
  max-width: 140px;
}
