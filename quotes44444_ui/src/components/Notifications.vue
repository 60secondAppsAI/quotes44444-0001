...
const sortBy = ref('notificationId');
...
const loadNotifications = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/notification/notifications?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const getNotificationIcon = (notification: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[notification.notificationId % icons.length];
};

const viewNotificationDetails = (notification: any) => {
  router.push({ name: 'NotificationDetail', params: { id: notification.notificationId } });
};
  ...
  <div class="notifications-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="notifications-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="notifications-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <h3 class="error-title">Failed to Load Notifications</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadNotifications()" class="retry-button">
          Try Again
        </button>
      </div>
      ...
      <div v-else-if="filteredNotifications.length === 0" class="empty-state">
        ...
        <p class="empty-message">
          {{ searchQuery ? 'No notifications match your search criteria.' : 'No notifications found in the system.' }}
        </p>
      </div>

      <!-- Notifications Grid -->
      <div v-else-if="viewMode === 'cards'" class="notifications-grid">
        <div
        ...
        >
          ...
          <div class="notification-info">
            <h3 class="notification-name">{{ notification.name || 'Unknown Notification' }}</h3>
            ...
            <div class="notification-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">NotificationId:</span>
                <span class="metadata-value">{{ notification.notificationId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Message:</span>
                <span class="metadata-value">{{ notification.message || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">NotificationDate:</span>
                <span class="metadata-value">{{ notification.notificationDate || 'N/A' }}</span>
              </div>
      ...
      <!-- Notifications Table -->
      <div v-else-if="viewMode === 'table'" class="notifications-table-container">
        <table class="notifications-table">
          <thead>
            <tr>

              <th @click="handleSort('notificationId')" class="sortable-header">
                NotificationId {{ getSortIcon('notificationId') }}
              </th>
              <th @click="handleSort('message')" class="sortable-header">
                Message {{ getSortIcon('message') }}
              </th>
              <th @click="handleSort('notificationDate')" class="sortable-header">
                NotificationDate {{ getSortIcon('notificationDate') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="notification in filteredNotifications" :key="notification.notificationId" class="notification-row">
              
              
                      <td class="notification-id-cell" @click="viewNotificationDetails(notification)">{{ notification.notificationId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ notification.message }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ notification.notificationDate }}</span>
                </div>
              </td>
