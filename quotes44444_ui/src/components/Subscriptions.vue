...
const sortBy = ref('subscriptionId');
...
const loadSubscriptions = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/subscription/subscriptions?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadSubscriptions(currentPage.value + 1);
  }
};
...
const getSubscriptionIcon = (subscription: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[subscription.subscriptionId % icons.length];
};

const viewSubscriptionDetails = (subscription: any) => {
  router.push({ name: 'SubscriptionDetail', params: { id: subscription.subscriptionId } });
};
  ...
  <div class="subscriptions-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="subscriptions-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="subscriptions-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <h3 class="error-title">Failed to Load Subscriptions</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadSubscriptions()" class="retry-button">
          Try Again
        </button>
      </div>
      ...
      <div v-else-if="filteredSubscriptions.length === 0" class="empty-state">
        ...
        <p class="empty-message">
          {{ searchQuery ? 'No subscriptions match your search criteria.' : 'No subscriptions found in the system.' }}
        </p>
      </div>

      <!-- Subscriptions Grid -->
      <div v-else-if="viewMode === 'cards'" class="subscriptions-grid">
        <div
        ...
        >
          ...
          <div class="subscription-info">
            <h3 class="subscription-name">{{ subscription.name || 'Unknown Subscription' }}</h3>
            ...
            <div class="subscription-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">SubscriptionId:</span>
                <span class="metadata-value">{{ subscription.subscriptionId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">StartDate:</span>
                <span class="metadata-value">{{ subscription.startDate || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">EndDate:</span>
                <span class="metadata-value">{{ subscription.endDate || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">IsActive:</span>
                <span class="metadata-value">{{ subscription.isActive || 'N/A' }}</span>
              </div>
      ...
      <!-- Subscriptions Table -->
      <div v-else-if="viewMode === 'table'" class="subscriptions-table-container">
        <table class="subscriptions-table">
          <thead>
            <tr>

              <th @click="handleSort('subscriptionId')" class="sortable-header">
                SubscriptionId {{ getSortIcon('subscriptionId') }}
              </th>
              ...
              <th @click="handleSort('isActive')" class="sortable-header">
          ...
          <tbody>
            <tr v-for="subscription in filteredSubscriptions" :key="subscription.subscriptionId" class="subscription-row">
              
              
                      <td class="subscription-id-cell" @click="viewSubscriptionDetails(subscription)">{{ subscription.subscriptionId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ subscription.startDate }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ subscription.endDate }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ subscription.isActive }}</span>
                </div>
              </td>
