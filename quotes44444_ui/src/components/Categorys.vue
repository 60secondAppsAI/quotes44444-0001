...
const loadCategorys = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/category/categorys?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
    ...
    error.value = err.message || 'Failed to load categorys';
  }
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadCategorys(currentPage.value + 1);
  }
};
...
const getCategoryIcon = (category: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[category.categoryId % icons.length];
};

const viewCategoryDetails = (category: any) => {
...
<template>
  <div class="categorys-page">
    <AppHeader />
    
    <header class="categorys-header">
      <div class="header-row">
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              <h1 class="page-title">Categorys</h1>
              <div class="categorys-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="categorys-content">
      <!-- Loading State -->
      <div v-if="isLoading" class="loading-state">
        <div class="loading-spinner-large"></div>
        <p class="loading-text">Loading categorys...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="error-state">
        <div class="error-icon">⚠️</div>
        <h3 class="error-title">Failed to Load Categorys</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadCategorys()" class="retry-button">
          Try Again
        </button>
      </div>

      <!-- Empty State -->
      <div v-else-if="filteredCategorys.length === 0" class="empty-state">
        <div class="empty-icon">👥</div>
        <h3 class="empty-title">No Categorys Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No categorys match your search criteria.' : 'No categorys found in the system.' }}
        </p>
      </div>

      <!-- Categorys Grid -->
      <div v-else-if="viewMode === 'cards'" class="categorys-grid">
        <div
        ...
        >
          ...
          <div class="category-info">
            <h3 class="category-name">{{ category.name || 'Unknown Category' }}</h3>
            ...
            <div class="category-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">CategoryId:</span>
                <span class="metadata-value">{{ category.categoryId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Name:</span>
                <span class="metadata-value">{{ category.name || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Description:</span>
                <span class="metadata-value">{{ category.description || 'N/A' }}</span>
              </div>
      ...
      <!-- Categorys Table -->
      <div v-else-if="viewMode === 'table'" class="categorys-table-container">
        <table class="categorys-table">
          <thead>
            <tr>

              <th @click="handleSort('categoryId')" class="sortable-header">
                CategoryId {{ getSortIcon('categoryId') }}
              </th>
          ...
          <tbody>
            <tr v-for="category in filteredCategorys" :key="category.categoryId" class="category-row">
              
              
                      <td class="category-id-cell" @click="viewCategoryDetails(category)">{{ category.categoryId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ category.name }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ category.description }}</span>
                </div>
              </td>
...
.categorys-table th:first-child,
.categorys-table td:first-child {
  width: 140px;
  min-width: 140px;
  max-width: 140px;
}
