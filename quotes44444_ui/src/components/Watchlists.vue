...
const loadWatchlists = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/watchlist/watchlists?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
    ...
    error.value = err.message || 'Failed to load watchlists';
  }
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadWatchlists(currentPage.value + 1);
  }
};
...
const viewWatchlistDetails = (watchlist: any) => {
  router.push({ name: 'WatchlistDetail', params: { id: watchlist.watchlistId } });
};
...
<template>
  <div class="watchlists-page">
    ...
    <header class="watchlists-header">
      <div class="header-row">
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              <h1 class="page-title">Watchlists</h1>
              <div class="watchlists-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="watchlists-content">
      <!-- Loading State -->
      <div v-if="isLoading" class="loading-state">
        <div class="loading-spinner-large"></div>
        <p class="loading-text">Loading watchlists...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="error-state">
        <div class="error-icon">⚠️</div>
        <h3 class="error-title">Failed to Load Watchlists</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadWatchlists()" class="retry-button">
          Try Again
        </button>
      </div>

      <!-- Empty State -->
      <div v-else-if="filteredWatchlists.length === 0" class="empty-state">
        <div class="empty-icon">👥</div>
        <h3 class="empty-title">No Watchlists Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No watchlists match your search criteria.' : 'No watchlists found in the system.' }}
        </p>
      </div>

      <!-- Watchlists Grid -->
      <div v-else-if="viewMode === 'cards'" class="watchlists-grid">
        <div
          v-for="watchlist in filteredWatchlists"
          :key="watchlist.watchlistId"
          class="watchlist-card"
          @click="viewWatchlistDetails(watchlist)"
        >
          ...
          <div class="watchlist-info">
            <h3 class="watchlist-name">{{ watchlist.name || 'Unknown Watchlist' }}</h3>
            <p class="watchlist-description">
              Watchlist ID: {{ watchlist.watchlistId }}
            </p>
            
            <div class="watchlist-metadata">

              <div class="metadata-item">
                <span class="metadata-label">WatchlistId:</span>
                <span class="metadata-value">{{ watchlist.watchlistId || 'N/A' }}</span>
              </div>
      ...
      <!-- Watchlists Table -->
      <div v-else-if="viewMode === 'table'" class="watchlists-table-container">
        <table class="watchlists-table">
          <thead>
            <tr>

              <th @click="handleSort('watchlistId')" class="sortable-header">
                WatchlistId {{ getSortIcon('watchlistId') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="watchlist in filteredWatchlists" :key="watchlist.watchlistId" class="watchlist-row">
              
              
                      <td class="watchlist-id-cell" @click="viewWatchlistDetails(watchlist)">{{ watchlist.watchlistId }}</td>
...
.watchlists-table th:first-child,
.watchlists-table td:first-child {
  width: 140px;
  min-width: 140px;
  max-width: 140px;
}
