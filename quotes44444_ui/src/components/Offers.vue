...
const loadOffers = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/offer/offers?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadOffers(currentPage.value + 1);
  }
};
...
const getOfferIcon = (offer: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[offer.offerId % icons.length];
};

const viewOfferDetails = (offer: any) => {
  router.push({ name: 'OfferDetail', params: { id: offer.offerId } });
};
  ...
  <div class="offers-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="offers-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="offers-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <h3 class="error-title">Failed to Load Offers</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadOffers()" class="retry-button">
          Try Again
        </button>
      </div>
      ...
      <div v-else-if="filteredOffers.length === 0" class="empty-state">
        ...
        <p class="empty-message">
          {{ searchQuery ? 'No offers match your search criteria.' : 'No offers found in the system.' }}
        </p>
      </div>

      <!-- Offers Grid -->
      <div v-else-if="viewMode === 'cards'" class="offers-grid">
        <div
        ...
        >
          ...
          <div class="offer-info">
            <h3 class="offer-name">{{ offer.name || 'Unknown Offer' }}</h3>
            ...
            <div class="offer-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">OfferId:</span>
                <span class="metadata-value">{{ offer.offerId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">OfferAmount:</span>
                <span class="metadata-value">{{ offer.offerAmount || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">OfferDate:</span>
                <span class="metadata-value">{{ offer.offerDate || 'N/A' }}</span>
              </div>
      ...
      <!-- Offers Table -->
      <div v-else-if="viewMode === 'table'" class="offers-table-container">
        <table class="offers-table">
          <thead>
            <tr>

              <th @click="handleSort('offerId')" class="sortable-header">
                OfferId {{ getSortIcon('offerId') }}
              </th>
              <th @click="handleSort('offerAmount')" class="sortable-header">
                OfferAmount {{ getSortIcon('offerAmount') }}
              </th>
              <th @click="handleSort('offerDate')" class="sortable-header">
                OfferDate {{ getSortIcon('offerDate') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="offer in filteredOffers" :key="offer.offerId" class="offer-row">
              
              
                      <td class="offer-id-cell" @click="viewOfferDetails(offer)">{{ offer.offerId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ offer.offerAmount }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ offer.offerDate }}</span>
                </div>
              </td>
