...
const loadDisputes = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/dispute/disputes?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadDisputes(currentPage.value + 1);
  }
};
...
const getDisputeIcon = (dispute: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[dispute.disputeId % icons.length];
};

const viewDisputeDetails = (dispute: any) => {
  router.push({ name: 'DisputeDetail', params: { id: dispute.disputeId } });
};
  ...
  <div class="disputes-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="disputes-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="disputes-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <h3 class="error-title">Failed to Load Disputes</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadDisputes()" class="retry-button">
          Try Again
        </button>
      </div>
      ...
      <div v-else-if="filteredDisputes.length === 0" class="empty-state">
        ...
        <h3 class="empty-title">No Disputes Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No disputes match your search criteria.' : 'No disputes found in the system.' }}
        </p>
      </div>

      <!-- Disputes Grid -->
      <div v-else-if="viewMode === 'cards'" class="disputes-grid">
        <div
        ...
        >
          <div class="dispute-header">
            <div class="dispute-icon">
              {{ getDisputeIcon(dispute) }}
            </div>
          </div>

          <div class="dispute-info">
            <h3 class="dispute-name">{{ dispute.name || 'Unknown Dispute' }}</h3>
            ...
            <div class="dispute-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">DisputeId:</span>
                <span class="metadata-value">{{ dispute.disputeId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Reason:</span>
                <span class="metadata-value">{{ dispute.reason || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">DisputeDate:</span>
                <span class="metadata-value">{{ dispute.disputeDate || 'N/A' }}</span>
              </div>
      ...
      <!-- Disputes Table -->
      <div v-else-if="viewMode === 'table'" class="disputes-table-container">
        <table class="disputes-table">
          <thead>
            <tr>

              <th @click="handleSort('disputeId')" class="sortable-header">
                DisputeId {{ getSortIcon('disputeId') }}
              </th>
              <th @click="handleSort('reason')" class="sortable-header">
                Reason {{ getSortIcon('reason') }}
              </th>
              <th @click="handleSort('disputeDate')" class="sortable-header">
                DisputeDate {{ getSortIcon('disputeDate') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="dispute in filteredDisputes" :key="dispute.disputeId" class="dispute-row">
              
              
                      <td class="dispute-id-cell" @click="viewDisputeDetails(dispute)">{{ dispute.disputeId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ dispute.reason }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ dispute.disputeDate }}</span>
                </div>
              </td>
...
.disputes-table th:first-child,
.disputes-table td:first-child {
  width: 140px;
  min-width: 140px;
  max-width: 140px;
}
