...
const loadTransactions = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/transaction/transactions?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const getTransactionIcon = (transaction: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[transaction.transactionId % icons.length];
};

const viewTransactionDetails = (transaction: any) => {
  router.push({ name: 'TransactionDetail', params: { id: transaction.transactionId } });
};
  ...
  <div class="transactions-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="transactions-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="transactions-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <h3 class="error-title">Failed to Load Transactions</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadTransactions()" class="retry-button">
          Try Again
        </button>
      </div>
      ...
      <div v-else-if="filteredTransactions.length === 0" class="empty-state">
        ...
        <p class="empty-message">
          {{ searchQuery ? 'No transactions match your search criteria.' : 'No transactions found in the system.' }}
        </p>
      </div>

      <!-- Transactions Grid -->
      <div v-else-if="viewMode === 'cards'" class="transactions-grid">
        <div
        ...
        >
          ...
          <div class="transaction-info">
            <h3 class="transaction-name">{{ transaction.name || 'Unknown Transaction' }}</h3>
            ...
            <div class="transaction-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">TransactionId:</span>
                <span class="metadata-value">{{ transaction.transactionId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">TransactionAmount:</span>
                <span class="metadata-value">{{ transaction.transactionAmount || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">TransactionDate:</span>
                <span class="metadata-value">{{ transaction.transactionDate || 'N/A' }}</span>
              </div>
      ...
      <!-- Transactions Table -->
      <div v-else-if="viewMode === 'table'" class="transactions-table-container">
        <table class="transactions-table">
          <thead>
            <tr>

              <th @click="handleSort('transactionId')" class="sortable-header">
                TransactionId {{ getSortIcon('transactionId') }}
              </th>
              <th @click="handleSort('transactionAmount')" class="sortable-header">
                TransactionAmount {{ getSortIcon('transactionAmount') }}
              </th>
              <th @click="handleSort('transactionDate')" class="sortable-header">
                TransactionDate {{ getSortIcon('transactionDate') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="transaction in filteredTransactions" :key="transaction.transactionId" class="transaction-row">
              
              
                      <td class="transaction-id-cell" @click="viewTransactionDetails(transaction)">{{ transaction.transactionId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ transaction.transactionAmount }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ transaction.transactionDate }}</span>
                </div>
              </td>
