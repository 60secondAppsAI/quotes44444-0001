...
const loadBids = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/bid/bids?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
    ...
    error.value = err.message || 'Failed to load bids';
  }
...
const getBidIcon = (bid: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[bid.bidId % icons.length];
};

const viewBidDetails = (bid: any) => {
  router.push({ name: 'BidDetail', params: { id: bid.bidId } });
};
  ...
  <div class="bids-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="bids-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="bids-content">
      ...
      <div v-if="isLoading" class="loading-state">
        ...
        <p class="loading-text">Loading bids...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="error-state">
        <div class="error-icon">⚠️</div>
        <h3 class="error-title">Failed to Load Bids</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadBids()" class="retry-button">
          Try Again
        </button>
      </div>
      ...
      <div v-else-if="filteredBids.length === 0" class="empty-state">
        ...
        <h3 class="empty-title">No Bids Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No bids match your search criteria.' : 'No bids found in the system.' }}
        </p>
      </div>

      <!-- Bids Grid -->
      <div v-else-if="viewMode === 'cards'" class="bids-grid">
        <div
        ...
        >
          ...
          <div class="bid-info">
            <h3 class="bid-name">{{ bid.name || 'Unknown Bid' }}</h3>
            ...
            <div class="bid-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">BidId:</span>
                <span class="metadata-value">{{ bid.bidId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">BidAmount:</span>
                <span class="metadata-value">{{ bid.bidAmount || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">BidDate:</span>
                <span class="metadata-value">{{ bid.bidDate || 'N/A' }}</span>
              </div>
      ...
      <!-- Bids Table -->
      <div v-else-if="viewMode === 'table'" class="bids-table-container">
        <table class="bids-table">
          <thead>
            <tr>

              <th @click="handleSort('bidId')" class="sortable-header">
                BidId {{ getSortIcon('bidId') }}
              </th>
              <th @click="handleSort('bidAmount')" class="sortable-header">
                BidAmount {{ getSortIcon('bidAmount') }}
              </th>
              <th @click="handleSort('bidDate')" class="sortable-header">
                BidDate {{ getSortIcon('bidDate') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="bid in filteredBids" :key="bid.bidId" class="bid-row">
              
              
                      <td class="bid-id-cell" @click="viewBidDetails(bid)">{{ bid.bidId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ bid.bidAmount }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ bid.bidDate }}</span>
                </div>
              </td>
