...
const loadInvoices = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/invoice/invoices?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadInvoices(currentPage.value + 1);
  }
};
...
const getInvoiceIcon = (invoice: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[invoice.invoiceId % icons.length];
};

const viewInvoiceDetails = (invoice: any) => {
  router.push({ name: 'InvoiceDetail', params: { id: invoice.invoiceId } });
};
  ...
  <div class="invoices-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="invoices-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="invoices-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <h3 class="error-title">Failed to Load Invoices</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadInvoices()" class="retry-button">
          Try Again
        </button>
      </div>
      ...
      <div v-else-if="filteredInvoices.length === 0" class="empty-state">
        ...
        <p class="empty-message">
          {{ searchQuery ? 'No invoices match your search criteria.' : 'No invoices found in the system.' }}
        </p>
      </div>

      <!-- Invoices Grid -->
      <div v-else-if="viewMode === 'cards'" class="invoices-grid">
        <div
        ...
        >
          ...
          <div class="invoice-info">
            <h3 class="invoice-name">{{ invoice.name || 'Unknown Invoice' }}</h3>
            ...
            <div class="invoice-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">InvoiceId:</span>
                <span class="metadata-value">{{ invoice.invoiceId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">IssueDate:</span>
                <span class="metadata-value">{{ invoice.issueDate || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">TotalAmount:</span>
                <span class="metadata-value">{{ invoice.totalAmount || 'N/A' }}</span>
              </div>
        ...
        <table class="invoices-table">
          <thead>
            <tr>
              ...
              <th @click="handleSort('invoiceId')" class="sortable-header">
                InvoiceId {{ getSortIcon('invoiceId') }}
              </th>
              <th @click="handleSort('issueDate')" class="sortable-header">
                IssueDate {{ getSortIcon('issueDate') }}
              </th>
              <th @click="handleSort('totalAmount')" class="sortable-header">
                TotalAmount {{ getSortIcon('totalAmount') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="invoice in filteredInvoices" :key="invoice.invoiceId" class="invoice-row">
              
              
                      <td class="invoice-id-cell" @click="viewInvoiceDetails(invoice)">{{ invoice.invoiceId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ invoice.issueDate }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ invoice.totalAmount }}</span>
                </div>
              </td>
